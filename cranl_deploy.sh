...
REPO_ID="23bd2e35-51ff-48ba-9dff-85f0e35b7194" # gulfboost/AI-interior-design from your repos

echo "=== Deploying Backend ==="
# cranl apps create --repo $REPO_ID --name ai-styling-backend --build-type nixpacks --branch master
...
# Database 
# (Since we are using Supabase Postgres, ensure DATABASE_URL is set in local .env, we will sync it)
cranl apps env set ai-styling-backend DATABASE_URL=$DATABASE_URL

echo "=== Deploying Frontend ==="
# cranl apps create --repo $REPO_ID --name ai-styling-frontend --build-type nixpacks --branch master
